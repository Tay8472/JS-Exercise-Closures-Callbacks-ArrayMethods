...
function isItAnApple(strings) {
  return strings.map(function(fart){
    return fart === 'apple';
  })
}
...
function stringSmash(strings) {
  // return strings.reduce(function (collector, item) {
  //   return collector + item;
  // });
  return strings.reduce((collector, item) => collector += item);
}
...
function firstNamesAllCaps(runners) {
  return runners.map(runner => runner.toUpperCase());
}
 ...
 * 
 * counter1 is a closure and counter2 is a global counter. counter1 can be restricted and counter2 could in theory be mutated by users.
 * 
 * 2. Which of the two uses a closure? How can you tell?
 * 
 * counter1 utilizes a closure because it returns a count variable inside a function
 * 
 * 3. In what scenario would the counter1 code be preferable? In what scenario would counter2 be better? 
 *
 * counter1 would be useful for counting specific things, counter2 would be more useful for a community to edit
*/
...
const mikesTurdHat = counterMaker();
mikesTurdHat();
